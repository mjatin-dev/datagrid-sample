...
const QuestionnaireForm = ({ next, stepper, values , data, setData }) => {
  ...
  const subDropDownDeadline = (
    ...
    let temp = [...data];
    if (date) {
      let date1 = new Date(values.start_date);
      let date2 = new Date(date.format("YYYY-MM-DD"));
      let dateShouldBeBetween = getAssignmentEndDate(
        values.start_date,
        bp,
        doc
      );
      ...
      if (!(date2 >= date1) || date2 > date3) {
        ...
        temp[sectionIndex].questions[
          dataIndex
        ].deadLineDateError = `Date Should be between ${values.start_date} and ${dateShouldBeBetween}`;
      } else {
  ...
  const dropDownChangeDeadline = (date, dateString, sectionIndex, bp, doc) => {
    ...
    if (date) {
      ...
      if (!(date2 >= date1) || date2 > date3) {
        ...
        temp[sectionIndex].questions?.map((item, i) => {
          item.error = true;
          item.deadLineDateError = `Date Should be between ${values.start_date} and ${dateShouldBeBetween}`;
        });
      } else {
        temp[sectionIndex].error = false;
        temp[sectionIndex].deadLineDateError = "";
        temp[sectionIndex].questions?.map((item) => {
          item.error = false;
  ...
  const onDataSubmit = () => {
    ...
    formData.append(
      "assignment_id",
      state?.AuditReducer?.assignmentId || assignment_id
    );
    ...
    <div className={`${styles.questionnaireForm}`} ref={selectRef}>
      ...
      <div className={styles.questionnaireFormHeader}>
        ...
        <div className={styles.questionnaireFormHeaderOptions}>
          <div
            className={`${styles.inputGroupRowReverse} dropDownPortalForForm`}
          >
            <CreatableSelect
              styles={customStyle}
              placeholder="Select"
                ...
                <tr>
                  <td className={styles.questionnaireFormSectionName}>
                    ...
                    <IconButton
                      ...
                      icon={items.expand ? <MdExpandMore /> : <MdExpandLess />}
                      size="none"
                    ...
                    <DatePicker
                      ...
                      disabledDate={(current) => {
                        ...
                        return (
                          ...
                          (current && current.isSameOrAfter(endDate, "day")) ||
                                  ...
                                  <td>
                                    <DatePicker
                                      ...
                                      disabledDate={(current) => {
                                        ...
                                        const endDate = getAssignmentEndDate(
                                          values.start_date,
                                          item.buffer_period,
                                          item.duration_of_completion
                                        );
