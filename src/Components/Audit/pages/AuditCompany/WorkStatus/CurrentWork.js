...
function CurrentWork() {
  ...
  const getCompanyCurrentWork = async () => {
    try {
      const resp = await axiosInstance.post(
        "audit.api.getCompanyWiseCurrentWork",
        { company: routeId }
      );
      if (resp) {
        const { data } = resp;
        if (data?.message?.status) {
          setCurrentWorkData(data.message.data);
        }
        setIsLoading(false);
      }
    } catch (error) {
  ...
  const onRedirect = (data, type = "questionnaire") => {
    ...
    history.push({
      pathname: `${path}/current-work/questionaire-checklist`,
      state: {
        company: company,
    ...
    <>
      ...
      <div className="table-cell" ref={tableRef}>
        <DataGrid
          ...
          onExporting={(e) =>
            exportValidation(currentWorkData?.length, e, `Current Work`)
          }
          export={{
