...
function Leave({ closeLeave, onClose, open }) {
  ...
  const applyForLeave = () => {
    ...
    formData.append("reference_attachment_files",acceptedFiles[0])
    try {
      axiosInstance.post("audit.api.ApplyLeave", formData).then((res) => {
        ...
        >
          <div className={styles.box}>
            ...
            {acceptedFiles.length > 0 ? (
              <div className={styles.afterFileUpload}>
                <h1>
                {acceptedFiles[0].name}
                </h1>
                </div>
            ) : (
              <div className={styles.leavDrop}>
                <div {...getRootProps()} className={styles.textData}>
                  <input {...getInputProps()} />
