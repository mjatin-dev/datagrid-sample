...
const NewRegulationFilter = (props) => {
  ...
  const { industryList, issuerList, topicList, isLoading } =
    state.UpdatesReducer;

  useEffect(() => {
    //set industry list for searchable dropdown.
    var setArrayOfObjectInList = industryList
      ?.filter((item) => item)
      ?.map((item) => {
    ...
    setArrayOfObjectInList = issuerList
      ?.filter((item) => item)
      ?.map((item) => {
        return { value: item, label: item };
      });
    ...
    setArrayOfObjectInList = topicList
      ?.filter((item) => item.topic)
      .map((itemValue) => {
        const { topic } = itemValue;
  ...
  const getTopicListByIssuer = () => {
    const selectedIssuers = state.UpdatesReducer.issuer
    ...
    if (selectedIssuers?.length > 0) {
      return (
        [...(topicList || [])]
          .filter((topicData) => {
            const isInclude = selectedIssuers.find((el) =>
              [...(topicData?.issuer || [])]?.includes(el)
            );
            return isInclude;
          })
          ?.map((topicData) => {
