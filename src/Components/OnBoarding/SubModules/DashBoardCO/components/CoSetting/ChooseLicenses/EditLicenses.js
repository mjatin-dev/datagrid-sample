...
const EditLicense = ({
  ...
  useEffect(() => {
    const selectedLicense =
      ...
      state?.PaymentReducer?.selectedLicense;
    if (selectedLicense.length > 0) {
      const mapLicnese = selectedLicense.map((license) => license.license);
      const mapCompany = selectedLicense.map((license) => license.company_id);
      getOwnLicense(mapLicnese, mapCompany);
    }
  }, [state?.PaymentReducer?.selectedListOfLicenses]);
  ...
  const getOwnLicense = async (mapLicnese, mapCompany) => {
    ...
    if (licenseList) {
      ...
      const newLicenseList = message?.map((values) => {
        return {
          ...
          show: mapCompany.includes(values.company_id),
          selected: mapCompany.includes(values.company_id),
          company_id: values.company_id,
          license: values.license.map((licenseValue) => {
            return {
              license: licenseValue,
              total_task: 0,
              show: mapLicnese.includes(licenseValue),
              company_id: values.company_id,
              ...
              selected:
                ...
                fields.selectedLiecenseIdArray.includes(licenseValue)
                  ? true
                  : mapCompany.includes(values.company_id) &&
                    mapLicnese.includes(licenseValue)
                  ? true
                  : false,
            };
          }),
    ...
    <>
      ...
      <div>
        <div className="px-4 choose-license__top">
          <p className="d-none d-md-block h3 mb-3">Edit license you have</p>
        </div>
