...
function ScheduledTasks({ history }) {
    ...
    if (taskList && taskList.length > 0) {
      ...
      const schedulededTask = getAllTasks(taskList).filter(
        (task) =>
          ...
          new Date(task?.due_date || task.deadline_date) >=
            new Date(startOfMonth) &&
          new Date(task?.due_date || task.deadline_date) <= new Date(endOfMonth)
      );

      setPendingArr([...schedulededTask]);
    }
  }, [click, taskList]);
            ...
            <div className="task-list-grid">
              <div className="upcoming-btn-pending">
                <div className="pink-circle-closing">
                  <div className="pink-label-close">Scheduled Tasks</div>
                  <img
...
export default withRouter(ScheduledTasks);
