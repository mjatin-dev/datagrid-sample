...
import { actions as signUpActions } from "Components/Authectication/redux/actions";
import api from "../api";
...
const insertUpdateDeleteAPIReq = function* insertUpdateDeleteAPIReq({
  ...
  try {
    ...
    const { data } = yield call(api.checkEmailVerifiedThroughEmail, payload);
    const { message } = data;
    if (message.status !== false) {
      const { token, auth_token } = message;
      localStorage.setItem("basicToken", auth_token);
      toast.success("Personal Information saved successfully");

      console.log(message);
      yield put(signUpActions.updateCompanyExists(true));
...
const companyTypeRequest = function* companyTypeRequest({ payload }) {
  try {
    const { data } = yield call(api.companyType, payload);
...
const getLicenseList = function* getLicenseList({ payload }) {
  try {
    const { data } = yield call(api.getLicenseList, payload);
    if (data) {
      yield put(
...
const insertCerificateRequest = function* insertCerificateRequest({ payload }) {
  try {
    ...
    if (data && data.message && data?.message?.status && status === 200) {
      ...
      payload?.history.push("/otpverification-co");
    } else {
...
const insertTaskListReq = function* insertTaskListReq({ payload }) {
  try {
    const { status } = yield call(api.insertTempTask, payload);
