...
const insertUpdateDeleteAPIReqTM = function* insertUpdateDeleteAPIReqTM({
  payload,
}) {
  try {
    ...
    if (status === 200) {
      ...
      if (statusCode !== undefined && !statusCode) {
        yield put(
          actions.insUpdateDeletAPIRequestSuccessTM({
            loginSuccess: true,
            statusCode: status,
            message: message,
          })
...
const emailavailabilityCheckReq = function* emailavailabilityCheckReq({
  payload,
}) {
  try {
...
export default function* sagas() {
  yield takeLatest(
    types.INS_UPDATE_DELETE_API_REQUEST_TM,
    insertUpdateDeleteAPIReqTM
  );
