...
function PersonalDetails({ history }) {
  ...
  const reference_id = new URLSearchParams(search).get("reference_id");
  ...
  const onChangeHandler = (name) => (event) => {
    ...
    if (
      name === "companyName" &&
      !/^[a-zA-Z0-9 ]{0,}$/.test(event.target.value)
    ) {
      return;
    }
    if (name === "countryCode") {
  ...
  useEffect(() => {
    // Reset user state and loading
    dispatch(personalDetailsAction.userDataSaveRequestFailed());
  }, []);
  return (
    <div className="row">
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-6 col-xs-12">
                        <div className="form-group">
                          ...
                          <ul className="Instruction" style={{ marginTop: 10 }}>
                            <li>
                              <div
                                className={
